...
| D6 | **DONE** (explicitly requested post-D3). CodeMap behind `codemap.enabled: false`: incremental hash-manifest summarizer in distill (`tb distill --codemap` + CI template), indexed under C4's reserved `source: 'codemap'`, served through the existing four MCP tools with an isolated 1500-token budget. Accept evidence: budget-isolation + staleness + disabled-flag + both-modes-e2e tests green; four-tool list snapshot unchanged; bench 500k-LOC incremental update 8.1s (<2 min budget). **Closed 2026-07-10:** the C2 `explore` kind was explicitly approved and implemented — Read/Grep/Glob captured as path-only metadata; the digest reports exploration/session with a codemap-vs-no-codemap reduction % (the §4.8 ≥30% instrument). |
